const App = {
  ...
  smartShuffle(questions, count) {
    const byDiff = { 1: [], 2: [], 3: [] };
    questions.forEach(q => {
      const d = q.dificuldade || 2;
      byDiff[d].push(q);
    });

    // Shuffle each group
    Object.values(byDiff).forEach(arr => {
    ...
    // Pick balanced: 1/3 easy, 1/3 medium, 1/3 hard
    const selected = [];
    const perDiff = Math.ceil(count / 3);
    for (const d of [1, 2, 3]) {
      const take = Math.min(perDiff, byDiff[d].length);
      selected.push(...byDiff[d].slice(0, take));
    }
    ...
    // Interleave by difficulty (like real ENEM: mixed order)
    const pools = { 1: [], 2: [], 3: [] };
    selected.forEach(q => pools[q.dificuldade || 2].push(q));

    const result = [];
    const diffOrder = [1, 2, 3];
    let idx = 0;
    while (result.length < count) {
      const d = diffOrder[idx % 3];
      if (pools[d].length > 0) {
        result.push(pools[d].shift());
      } else {
        // Find any remaining
        for (const dd of diffOrder) {
          if (pools[dd].length > 0) {
            result.push(pools[dd].shift());
            break;
          }
        }
      }
  ...
  renderTextWithImage(text, imagem) {
    if (!imagem) return this.esc(text);
    const imgHtml = this.renderImagemHtml(imagem);
    const placeholder = '§IMG§';
    // Replace first large gap of newlines (3+) with placeholder
    const prepared = text.replace(/\n{3,}/, '\n' + placeholder + '\n');
    // Escape (converts \n to <br>)
    let result = this.esc(prepared);
    // Replace placeholder with <br> + image + <br>
    result = result.replace(new RegExp('<br>' + placeholder.replace(/[§]/g, '\\$&') + '<br>'), '<br>' + imgHtml + '<br>');
    // Collapse remaining multiple <br> into double
  ...
  getSelectedDificuldades() {
    return Array.from(document.querySelectorAll('#dificuldade-chips .form-chip.selected')).map(c => parseInt(c.dataset.diff));
  },
  ...
  updateSubtopicos() {
    ...
    // Build list from SUBTOPICOS_OFICIAIS instead of dados.js
    const subs = [];
    for (const disc of disciplinas) {
      const official = SUBTOPICOS_OFICIAIS[disc];
      if (official) {
    ...
    subs.sort((a, b) => a.localeCompare(b, 'pt-BR'));

    if (subs.length === 0) {
      container.innerHTML = '<p class="form-hint">Nenhuma sub-matéria encontrada para a seleção atual.</p>';
      return;
    }
  ...
  async startPractice() {
    ...
    const dificuldades = this.getSelectedDificuldades();
    ...
    if (dificuldades.length > 0) pool = pool.filter(q => dificuldades.includes(q.dificuldade));
    ...
    this.currentAnswers = {};
    this.mode = 'questoes';
    this.sessionConfig = { areas, disciplinas, subtopicos, dificuldades, num };
            ...
            <div class="options-list">
              ${opcoes.map((opt, oi) => {
                ...
                return `<div class="option-item" data-q="${q.id}" data-letter="${letter}" onclick="App.selectOption('${q.id}','${letter}',this)">
                  <span class="option-letter">${letter}</span><span>${optContent}</span></div>`;
              }).join('')}
            </div>
          </div>
        </div>`;
        ...
        <div class="card-content" style="padding:1rem 1.25rem;">
          <div class="flex justify-between items-center mb-2 flex-wrap gap-1">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span class="badge badge-info">${DISCIPLINA_NAMES[q.disciplina]||q.disciplina}</span>
              ${q.subtopico?`<span class="badge badge-warning">${this.esc(q.subtopico)}</span>`:''}
        <span class="badge badge-default">${this.esc(q.fonte||'ENEM')}</span>
            </div>
